...
#     print("Time to Connect Participant",time() - config["s"])
...
#     print("Time To fetch Assistant Info",time() - config["s"])
...
#         tts = sarvam.TTS(
#             speaker=voice_id, target_language_code="en-IN", model="bulbul:v1"
#         )
...
#         gpt_llm = openai.LLM(model="gpt-4o-mini",parallel_tool_calls=True)
#     elif gpt_model == "gpt-4.1-nano-2025-04-14":
#         gpt_llm = openai.LLM(model="gpt-4.1-nano-2025-04-14",parallel_tool_calls=True)
#     else:
#         gpt_llm = openai.LLM(model="gpt-4o-mini",parallel_tool_calls=True)
...
#         stt=deepgram.STT(language="multi",model=dg_model),
#         llm=gpt_llm,
...
#     config["s"] = time()
#     await agent.say(
#         assistant_info.get("welcome_message_text"), allow_interruptions=True
#     )
...
#         print("Response Time", time() - config["s"])
...
#     cli.run_app(
#         WorkerOptions(
#             entrypoint_fnc=entrypoint, prewarm_fnc=prewarm, shutdown_process_timeout=2
#         ),
#     )
...
async def entrypoint(ctx: JobContext):
    ...
    agent = VoicePipelineAgent(
        vad=ctx.proc.userdata["vad"],
        stt=deepgram.STT(language="en-IN", model=dg_model,smart_format=False,punctuate=False,filler_words=False),
        llm=gpt_llm,
        before_tts_cb=replace_words,
        tts=tts,
        chat_ctx=initial_ctx,
